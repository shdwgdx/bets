...
function getMatchesSourcePafbetCsgo($url, $bookmaker = 'pafbet')
{
    ...
    try {
        ...
        if (isset($data->events)) {
            foreach ($data->events as $event) {
                if (strtolower(trim($event->event->path[1]->name)) == 'cs:go') {
                    $league_title = strtolower(trim($event->event->path[2]->name));
                    ...
                    if (isset($event->betOffers[0]->outcomes)) {
        ...
        // Обработка ошибки
        echo "Произошла ошибка: pafbetCsgo - $url" . $e->getMessage();
