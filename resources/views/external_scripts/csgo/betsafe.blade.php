            ...
            if (empty($values['Draw Odds'])) {
                ...
                foreach ($data['data']['events'] as $event) {
                    if ($event['participants'][0]['label'] == $team1 && $event['participants'][1]['label'] == $team2) {
                        ...
                        foreach ($data['data']['scoreboards'] as $scoreboard) {
                            if (strpos($scoreboard['eventId'], $event['id']) !== false) {
                                $score_team1 = array_values($scoreboard['scorePerParticipant'])[0];
                                $score_team2 = array_values($scoreboard['scorePerParticipant'])[1];
                                $live = true;
                                break;
                            }
                        }
                        ...
                        if (strpos($event['competitionName'], 'CS:') === 0) {
                            $league_title = strtolower(str_replace('CS: ', '', $event['competitionName']));
